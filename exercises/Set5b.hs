...
search val tree = searchHelper val tree []
  where
    ...
    searchHelper val Empty _ = Nothing
    searchHelper val (Node a left right) steps
      | val == a = Just steps
      | otherwise = case searchHelper val left (steps ++ [StepL]) of
                      Just path -> Just path
                      Nothing -> searchHelper val right (steps ++ [StepR])
